...
def run_walkforward(rows,min_train=45,max_combo=3,min_tenure=7,challenger_margin=0.010):
    ...
    meta_x=defaultdict(list); meta_y=defaultdict(list); discriminator_log=[]; shadow_events=defaultdict(list); boundary_log=[]; boundary_counts=Counter(); boundary_shadow=defaultdict(list); tournament_log=[]; tournament_counts=Counter(); fusion_shadow=defaultdict(list); fusion_log=[]
    ...
    for i in range(min_train,len(rows)):
        ...
        for house in HOUSES:
            ...
            if selected is None:
                primary_name='NO_QUALIFIED_PRIMARY'; primary_rank=None; canonical_primary_rank=None; primary_hash=None; secondary=None
                ...
                final_ranked=fusion_ranked if fusion_status['active'] else primary_ranked
                primary_rank=final_ranked.index(actual)+1
                hash_label='FUSION:'+primary_name if fusion_status['active'] else 'PRIMARY:'+primary_name
    ...
    return evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug,calibration_events,policy_history,discriminator_log,shadow_events,boundary_log,boundary_counts,boundary_shadow,tournament_log,tournament_counts,fusion_shadow,fusion_log
...
def report(rows,evals,combo_evals,timeline,switches,profiles,combo_profiles):
    ...
    rep['model']='DHAPPA v3.0 — Candidate-Level Cross-Route Fusion Intelligence'
...
def save_all(root,rows,evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug,calibration_events,policy_history,discriminator_log,shadow_events,boundary_log,boundary_counts,boundary_shadow,tournament_log,tournament_counts,fusion_shadow,fusion_log):
    ...
    (out/'dynamic_primary_engine_backtest_v3_0.json').write_text(json.dumps(rep,indent=2),encoding='utf-8')
    ...
    md[0]='# DHAPPA v3.0 — Candidate-Level Cross-Route Fusion Intelligence'
    ...
    (out/'DHAPPA_CANDIDATE_LEVEL_FUSION_V3_0_REPORT.md').write_text('\n'.join(md),encoding='utf-8')
    return rep
