...
def _select_house_policy(calibration_events, min_events=15):
    if len(calibration_events)<min_events:
        return 'BALANCED', {'n':len(calibration_events),'reason':'WARMUP_DEFAULT','scores':{},'survival':None}
    # Recent 60 challenger events plus expanding blend; all are completed prior targets.
    ...
    raw_best=ranked[0][0]
    # V2.4: a learned non-baseline policy must survive a prior-only shadow replay against BALANCED.
    # Survival is checked on recent and expanding evidence; the recent window protects regime relevance,
    # while expanding evidence prevents promotion from a tiny lucky patch.
    candidates=[]
    for name,_ in ranked:
        if name=='BALANCED':
            continue
        sr=_shadow_policy_compare(recent,name,'BALANCED')
        se=_shadow_policy_compare(calibration_events,name,'BALANCED')
        ok=(sr['survives'] and se['damaged_top5']<=se['damage_budget'] and se['net_top5']>=0 and se['net_top10']>=0 and se['mrr_delta']>=-0.001)
        candidates.append((name,ok,sr,se,scores[name]))
    survivor=next((x for x in candidates if x[1]),None)
    chosen=survivor[0] if survivor else 'BALANCED'
    chosen_survival=({'recent':survivor[2],'expanding':survivor[3]} if survivor else {'recent':_shadow_policy_compare(recent,'BALANCED'),'expanding':_shadow_policy_compare(calibration_events,'BALANCED')})
    return chosen, {'n':len(calibration_events),'reason':'PRIOR_ONLY_DAMAGE_AVERSE_POLICY_SURVIVAL','scores':scores,
                    'raw_best':raw_best,'chosen':chosen,'raw_margin':ranked[0][1]-ranked[1][1] if len(ranked)>1 else None,
                    'survival':chosen_survival,
                    'rejected_survival':[{ 'policy':n,'passed':ok,'recent':sr,'expanding':se } for n,ok,sr,se,_ in candidates if n!=chosen]}
...
def run_walkforward(rows,min_train=45,max_combo=3,min_tenure=7,challenger_margin=0.010):
    ...
    for i in range(min_train,len(rows)):
        ...
        for house in HOUSES:
            ...
            house_policy, policy_meta = _select_house_policy(calibration_events[house])
            ...
            if pending_calibration:
                ...
                policy_history.append({'date':target['date'],'house':house,'policy':house_policy,'meta':policy_meta,'event_count_after':len(calibration_events[house])})
...
def report(rows,evals,combo_evals,timeline,switches,profiles,combo_profiles):
    ...
    return {'model':'DHAPPA Dynamic Primary Engine v2.4','rows':len(rows),'date_range':[rows[0]['date'],rows[-1]['date']],
            ...
            'election_features':['champion_challenger_gating','15_30_60_expanding_agreement','minimum_tenure_hysteresis',
                                 'combination_survival_test','contextual_regime_modifier','NO_QUALIFIED_PRIMARY','paired_challenger_proof','asymmetric_champion_protection','incumbent_strength_tiers','damage_budget','fast_weak_challenger_path','failing_incumbent_abstention','house_specific_policy_learning','prior_only_policy_calibration','multi_objective_switch_utility','damage_averse_policy_survival','shadow_policy_replay','top5_damage_budget'],
...
def save_all(root,rows,evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug=None,calibration_events=None,policy_history=None):
    ...
    dump('dynamic_primary_engine_backtest_v2_4.json',rep)
    dump('primary_engine_election_timeline_v2_4.json',timeline)
    dump('single_engine_performance_v2_4.json',{h:{e:window_metrics(v) for e,v in profiles[h].items()} for h in HOUSES})
    dump('engine_combination_performance_v2_4.json',{h:{e:window_metrics(v) for e,v in combo_profiles[h].items()} for h in HOUSES})
    dump('dynamic_election_decisions_v2_4.json',election_debug or [])
    dump('house_policy_history_v2_4.json',policy_history or [])
    dump('house_policy_calibration_events_v2_4.json',{h:v for h,v in (calibration_events or {}).items()})
    failures=[]
    for x in timeline:
    ...
    dump('primary_engine_integrity_report_v2_4.json',integrity)
    md=['# DHAPPA Dynamic Primary Engine v2.4 Report','',f"Dataset: **{rep['rows']} rows**, {rep['date_range'][0]} → {rep['date_range'][1]}",'',
        '## V2.4 election architecture','House-specific policy learning is retained, but any non-BALANCED policy must now survive a prior-only shadow replay against BALANCED. Promotion requires positive net Top-5 rescue, a strict damaged-hit budget, non-negative Top-10 value, and acceptable MRR/rank behavior. A higher calibration utility alone is no longer enough.','',
    ...
    md += ['', '## Interpretation','V2.4 is allowed to abstain and learns its switch policy separately by house, but policy promotion is damage-averse: non-baseline policies must prove survival against BALANCED on prior completed challenger events. Therefore conditional hit rates on elected targets must be read together with election coverage; lower coverage can inflate conditional metrics. The all-target Top-5 measure is retained in JSON to prevent selective-reporting bias. Random ranking references remain Top-5=5%, Top-10=10%, Top-21=21%, Top-36=36%.','',
           '## Integrity conclusion','V2.4 preserves prior-only election ordering while adding a prior-only damage-averse survival test before a learned house policy may replace BALANCED. Election and route qualification are now performed entirely from previously completed targets, followed by immutable freeze and only then outcome evaluation.']
