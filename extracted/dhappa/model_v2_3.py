...
def _select_house_policy(calibration_events, min_events=15):
    if len(calibration_events)<min_events:
        return 'BALANCED', {'n':len(calibration_events),'reason':'WARMUP_DEFAULT','scores':{}}
    ...
    best=ranked[0][0]
    return best, {'n':len(calibration_events),'reason':'PRIOR_ONLY_HOUSE_CALIBRATION','scores':scores,'margin':ranked[0][1]-ranked[1][1] if len(ranked)>1 else None}
...
def report(rows,evals,combo_evals,timeline,switches,profiles,combo_profiles):
    ...
    return {'model':'DHAPPA Dynamic Primary Engine v2.3','rows':len(rows),'date_range':[rows[0]['date'],rows[-1]['date']],
...
def save_all(root,rows,evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug=None,calibration_events=None,policy_history=None):
    ...
    dump('dynamic_primary_engine_backtest_v2_3.json',rep)
    dump('primary_engine_election_timeline_v2_3.json',timeline)
    dump('single_engine_performance_v2_3.json',{h:{e:window_metrics(v) for e,v in profiles[h].items()} for h in HOUSES})
    dump('engine_combination_performance_v2_3.json',{h:{e:window_metrics(v) for e,v in combo_profiles[h].items()} for h in HOUSES})
    dump('dynamic_election_decisions_v2_3.json',election_debug or [])
    dump('house_policy_history_v2_3.json',policy_history or [])
    dump('house_policy_calibration_events_v2_3.json',{h:v for h,v in (calibration_events or {}).items()})
    failures=[]
    for x in timeline:
    ...
    dump('generation_vs_ranking_failure_v2_3.json',failures)
    integrity={'version':'v2.3','no_current_target_used_for_generation':True,'history_slice':'rows[:target_index]',
               'election_before_target_profile_update':True,'freeze_hash':'sha256(date,cutoff,house,engine,ranking)',
               'explicit_abstention_state':True,'paired_challenger_proof':True,'asymmetric_champion_protection':True,'house_specific_policy_learning':True,'policy_calibration_current_target_excluded':True,'strength_tiers':['STRONG','MODERATE','WEAK','FAILING'],'minimum_tenure':'tier-dependent: 7/5/3/2',
