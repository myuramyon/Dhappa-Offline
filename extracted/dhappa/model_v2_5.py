...
POLICY_GRID = {
    'CONSERVATIVE': {'strong_gap':.014,'moderate_gap':.010,'weak_gap':.005,'failing_gap':0.000,'tenure':(8,6,4,2),'proof_mode':'strict','switch_cost':.010},
    'BALANCED':     {'strong_gap':.010,'moderate_gap':.006,'weak_gap':.002,'failing_gap':-.003,'tenure':(7,5,3,2),'proof_mode':'balanced','switch_cost':.006},
    'ADAPTIVE':     {'strong_gap':.008,'moderate_gap':.004,'weak_gap':.000,'failing_gap':-.005,'tenure':(6,4,2,1),'proof_mode':'adaptive','switch_cost':.004},
    'FAST_RESCUE':  {'strong_gap':.006,'moderate_gap':.002,'weak_gap':-.002,'failing_gap':-.008,'tenure':(5,3,2,1),'proof_mode':'rescue','switch_cost':.002},
}

def _policy_switch_gate(policy_name,tier,candidate,incumbent,proof,tenure):
    p=POLICY_GRID[policy_name]; gap=candidate['score']-incumbent['score']
    idx={'STRONG':0,'MODERATE':1,'WEAK':2,'FAILING':3}[tier]
    min_tenure=p['tenure'][idx]
    if tenure < min_tenure:
        return False, f'{policy_name}_{tier}_MIN_TENURE'
    if tier=='STRONG':
        ok=(gap>=p['strong_gap'] and proof and proof['n']>=20 and proof['net_top5']>=0 and proof['mean_rank_delta']>0 and proof['mrr_delta']>=0)
        if p['proof_mode']=='strict': ok=ok and proof['qualified']
    elif tier=='MODERATE':
        ok=(gap>=p['moderate_gap'] and proof and proof['n']>=15 and proof['net_top5']>=0 and (proof['mean_rank_delta']>0 or proof['mrr_delta']>0))
        if p['proof_mode']=='strict': ok=ok and proof['mrr_delta']>=0
    elif tier=='WEAK':
        ok=(gap>=p['weak_gap'] and proof and proof['n']>=10 and proof['net_top5']>=0 and (proof['mean_rank_delta']>0 or proof['mrr_delta']>-.002))
    else:
        ok=(gap>=p['failing_gap'])
    return ok, f'{policy_name}_{tier}_' + ('SWITCH' if ok else 'RETAIN')

def _event_policy_switch(policy_name,event):
    # Replay a previously observed challenger event using only features frozen before that event.
    p=POLICY_GRID[policy_name]; tier=event['tier']; gap=event['gap']; tenure=event['tenure']; proof=event.get('proof')
    idx={'STRONG':0,'MODERATE':1,'WEAK':2,'FAILING':3}[tier]
    if tenure < p['tenure'][idx]: return False
    if tier=='STRONG':
        ok=(gap>=p['strong_gap'] and proof and proof['n']>=20 and proof['net_top5']>=0 and proof['mean_rank_delta']>0 and proof['mrr_delta']>=0)
        if p['proof_mode']=='strict': ok=ok and proof.get('qualified',False)
        return ok
    if tier=='MODERATE':
        ok=(gap>=p['moderate_gap'] and proof and proof['n']>=15 and proof['net_top5']>=0 and (proof['mean_rank_delta']>0 or proof['mrr_delta']>0))
        if p['proof_mode']=='strict': ok=ok and proof['mrr_delta']>=0
        return ok
    if tier=='WEAK':
        return bool(gap>=p['weak_gap'] and proof and proof['n']>=10 and proof['net_top5']>=0 and (proof['mean_rank_delta']>0 or proof['mrr_delta']>-.002))
    return gap>=p['failing_gap']

def _policy_event_utility(event,policy_name):
    switch=_event_policy_switch(policy_name,event)
    chosen=event['candidate_rank'] if switch else event['incumbent_rank']
    other=event['incumbent_rank'] if switch else event['candidate_rank']
    # Multi-objective utility: rescue useful ranks, reward reciprocal rank, penalize Top-5 damage and unnecessary switching.
    u=0.0
    u += 1.60*(chosen<=5) + .75*(chosen<=10) + .20*(chosen<=21) + .35*(1/chosen)
    if switch:
        u -= POLICY_GRID[policy_name]['switch_cost']
        if event['incumbent_rank']<=5 and event['candidate_rank']>5: u -= 1.25
        if event['incumbent_rank']>5 and event['candidate_rank']<=5: u += .75
    # Small continuous rank advantage to break ties without overwhelming hit tiers.
    u += .002*(other-chosen)
    return u

def _shadow_policy_compare(events, candidate, baseline='BALANCED'):
    """Compare a learned policy against BALANCED on already-completed challenger events only.
    Positive net metrics mean the candidate improves on the baseline policy's historical choices.
    """
    if not events:
        return {'n':0,'rescued_top5':0,'damaged_top5':0,'net_top5':0,'net_top10':0,'mrr_delta':0.0,'mean_rank_delta':0.0,'utility_delta':0.0,'survives':False,'reason':'NO_SHADOW_HISTORY'}
    rescued=damaged=net10=0
    mrr_delta=rank_delta=utility_delta=0.0
    for e in events:
        cs=_event_policy_switch(candidate,e); bs=_event_policy_switch(baseline,e)
        cr=e['candidate_rank'] if cs else e['incumbent_rank']
        br=e['candidate_rank'] if bs else e['incumbent_rank']
        rescued += int(br>5 and cr<=5)
        damaged += int(br<=5 and cr>5)
        net10 += int(cr<=10)-int(br<=10)
        mrr_delta += (1/cr)-(1/br)
        rank_delta += br-cr
        utility_delta += _policy_event_utility(e,candidate)-_policy_event_utility(e,baseline)
    n=len(events)
    net5=rescued-damaged
    # Damage budget is deliberately asymmetric: a candidate policy cannot buy rank/MRR gains
    # by sacrificing scarce baseline Top-5 hits. One damaged hit is tolerated only when there
    # are at least three rescues and net Top-5 remains clearly positive.
    damage_budget = 0 if rescued < 3 else max(0, rescued//4)
    survives=(candidate==baseline or (n>=20 and damaged<=damage_budget and net5>0 and net10>=0 and mrr_delta>=-0.002 and utility_delta>0))
    reason='BASELINE_POLICY' if candidate==baseline else ('DAMAGE_AVERSE_SURVIVAL_PASSED' if survives else 'DAMAGE_AVERSE_SURVIVAL_FAILED')
    return {'n':n,'rescued_top5':rescued,'damaged_top5':damaged,'damage_budget':damage_budget,'net_top5':net5,'net_top10':net10,
            'mrr_delta':mrr_delta/n,'mean_rank_delta':rank_delta/n,'utility_delta':utility_delta/n,'survives':survives,'reason':reason}

def _pareto_policy_survival(events, candidate, baselines):
    """Prior-only Pareto survival against multiple reference policies.
    Candidate must avoid unacceptable Top-5 damage versus EVERY distinct baseline,
    while showing strict value on at least one comparison. No dominance => retain incumbent.
    """
    refs=[]; strict_gain=False
    for baseline in dict.fromkeys(baselines):
        if not baseline or baseline==candidate: continue
        cmp=_shadow_policy_compare(events,candidate,baseline)
        # Shadow helper's survival flag was tuned for BALANCED; V2.5 applies explicit Pareto constraints.
        acceptable=(cmp['n']>=20 and cmp['damaged_top5']<=cmp['damage_budget'] and cmp['net_top5']>=0
                    and cmp['net_top10']>=0 and cmp['mrr_delta']>=-0.0015 and cmp['utility_delta']>=-0.002)
        strict=(cmp['net_top5']>0 or cmp['net_top10']>0 or cmp['mrr_delta']>0.001 or cmp['utility_delta']>0.002)
        strict_gain = strict_gain or strict
        refs.append({'baseline':baseline,'acceptable':acceptable,'strict_gain':strict,**cmp})
    survives=(all(x['acceptable'] for x in refs) and strict_gain) if refs else True
    return {'survives':survives,'comparisons':refs,'strict_gain':strict_gain,
            'reason':'PARETO_SURVIVAL_PASSED' if survives else 'PARETO_SURVIVAL_FAILED'}

def _select_house_policy(calibration_events, incumbent_policy='BALANCED', recent_surviving_policy='BALANCED', min_events=15):
    if len(calibration_events)<min_events:
        return incumbent_policy or 'BALANCED', {'n':len(calibration_events),'reason':'WARMUP_RETAIN_CURRENT_POLICY','scores':{},'survival':None,'chosen':incumbent_policy or 'BALANCED'}
    recent=calibration_events[-60:]
    scores={}
    for name in POLICY_GRID:
        er=sum(_policy_event_utility(e,name) for e in recent)/len(recent)
        ee=sum(_policy_event_utility(e,name) for e in calibration_events)/len(calibration_events)
        scores[name]=.70*er+.30*ee
    ranked=sorted(scores.items(),key=lambda x:(-x[1],x[0]))
    refs=[incumbent_policy or 'BALANCED', recent_surviving_policy or 'BALANCED', 'BALANCED']
    tested=[]
    chosen=None
    for name,_ in ranked:
        # Retaining current policy requires no promotion proof.
        if name==(incumbent_policy or 'BALANCED'):
            continue
        sr=_pareto_policy_survival(recent,name,refs)
        se=_pareto_policy_survival(calibration_events,name,refs)
        ok=(sr['survives'] and se['survives'])
        tested.append({'policy':name,'passed':ok,'recent':sr,'expanding':se,'score':scores[name]})
        if ok and chosen is None:
            chosen=name
    if chosen is None:
        chosen=incumbent_policy or 'BALANCED'
        reason='RETAIN_CURRENT_POLICY_NO_PARETO_DOMINANCE'
    else:
        reason='PRIOR_ONLY_PARETO_POLICY_PROMOTION'
    surv=next((x for x in tested if x['policy']==chosen),None)
    return chosen, {'n':len(calibration_events),'reason':reason,'scores':scores,'raw_best':ranked[0][0],
                    'chosen':chosen,'incumbent_policy':incumbent_policy,'recent_surviving_policy':recent_surviving_policy,
                    'raw_margin':ranked[0][1]-ranked[1][1] if len(ranked)>1 else None,
                    'survival':surv,'tested':tested}
...
def run_walkforward(rows,min_train=45,max_combo=3,min_tenure=7,challenger_margin=0.010):
    ...
    election_debug=[]
    calibration_events=defaultdict(list)
    policy_history=[]
    current_policy={h:'BALANCED' for h in HOUSES}
    recent_surviving_policy={h:'BALANCED' for h in HOUSES}
    ...
    for i in range(min_train,len(rows)):
        ...
        for house in HOUSES:
            ...
            house_policy, policy_meta = _select_house_policy(calibration_events[house], current_policy[house], recent_surviving_policy[house])
            prior_policy=current_policy[house]
            if house_policy != prior_policy:
                recent_surviving_policy[house]=house_policy
            ...
            if candidate is None:
                ...
                allow,reason=_policy_switch_gate(house_policy,tier,candidate,incumbent,paired_proof,tenure[house])
                if allow:
            ...
            if selected is None:
                ...
                election_debug.append({'date':target['date'],'house':house,'context':ctx,'decision':decision_reason,'selected':primary_name,
                                       ...
                                       'routes_considered':len(routes),'paired_proof':paired_proof,'incumbent_strength':incumbent_strength,'house_policy':house_policy,'policy_meta':policy_meta})
            else:
                ...
                election_debug.append({'date':target['date'],'house':house,'context':ctx,'decision':decision_reason,'selected':primary_name,
                                       ...
                                       'context_n':selected['context_n'],'context_mod':selected['context_mod'],'paired_proof':paired_proof,'incumbent_strength':incumbent_strength,'house_policy':house_policy,'policy_meta':policy_meta})

            # Record a challenger calibration event only AFTER this target's election is frozen.
            # The event becomes available to future targets only after the actual result is revealed below.
            pending_calibration=None
            if candidate and incumbent and candidate['name']!=incumbent_name and incumbent_strength:
                cand_ranked=rankings[candidate['members'][0]] if len(candidate['members'])==1 else fuse(rankings,candidate['members'],rel,indep)
                inc_ranked=rankings[incumbent['members'][0]] if len(incumbent['members'])==1 else fuse(rankings,incumbent['members'],rel,indep)
                pending_calibration={'date':target['date'],'tier':incumbent_strength['tier'],'gap':candidate['score']-incumbent['score'],
                    'tenure':tenure[house] if selected is None else max(1,tenure[house]),'proof':paired_proof,
                    'candidate':candidate['name'],'incumbent':incumbent_name,
                    'candidate_rank':cand_ranked.index(actual)+1,'incumbent_rank':inc_ranked.index(actual)+1,
                    'policy_used':house_policy}
            ...
            if pending_calibration:
                ...
                policy_history.append({'date':target['date'],'house':house,'policy':house_policy,'prior_policy':prior_policy,'recent_surviving_policy':recent_surviving_policy[house],'meta':policy_meta,'event_count_after':len(calibration_events[house])})
            ...
            timeline.append({'date':target['date'],'source_cutoff':cutoff,'house':house,'context':ctx,'primary':primary_name,
                             ...
                             'freeze_hash':primary_hash,'incumbent_strength':incumbent_strength['tier'] if incumbent_strength else None,'house_policy':house_policy})
    return evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug,calibration_events,policy_history
...
def report(rows,evals,combo_evals,timeline,switches,profiles,combo_profiles):
    ...
    return {'model':'DHAPPA Dynamic Primary Engine v2.5','rows':len(rows),'date_range':[rows[0]['date'],rows[-1]['date']],
            ...
            'election_features':['champion_challenger_gating','15_30_60_expanding_agreement','minimum_tenure_hysteresis',
                                 'combination_survival_test','contextual_regime_modifier','NO_QUALIFIED_PRIMARY','paired_challenger_proof','asymmetric_champion_protection','incumbent_strength_tiers','damage_budget','fast_weak_challenger_path','failing_incumbent_abstention','house_specific_policy_learning','prior_only_policy_calibration','multi_objective_switch_utility','damage_averse_policy_survival','shadow_policy_replay','top5_damage_budget','pareto_policy_survival','incumbent_policy_baseline','recent_survivor_baseline','RETAIN_CURRENT_POLICY'],
            'engine_count':len(ENGINES),'engines':[{'name':e.name,'family':e.family} for e in ENGINES],'house_metrics':house}

def save_all(root,rows,evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug=None,calibration_events=None,policy_history=None):
    ...
    dump('dynamic_primary_engine_backtest_v2_5.json',rep)
    dump('primary_engine_election_timeline_v2_5.json',timeline)
    dump('single_engine_performance_v2_5.json',{h:{e:window_metrics(v) for e,v in profiles[h].items()} for h in HOUSES})
    dump('engine_combination_performance_v2_5.json',{h:{e:window_metrics(v) for e,v in combo_profiles[h].items()} for h in HOUSES})
    dump('dynamic_election_decisions_v2_5.json',election_debug or [])
    dump('house_policy_history_v2_5.json',policy_history or [])
    dump('house_policy_calibration_events_v2_5.json',{h:v for h,v in (calibration_events or {}).items()})
    failures=[]
    for x in timeline:
    ...
    dump('generation_vs_ranking_failure_v2_5.json',failures)
    integrity={'version':'v2.5','no_current_target_used_for_generation':True,'history_slice':'rows[:target_index]',
               'election_before_target_profile_update':True,'freeze_hash':'sha256(date,cutoff,house,engine,ranking)',
               'explicit_abstention_state':True,'paired_challenger_proof':True,'asymmetric_champion_protection':True,'house_specific_policy_learning':True,'policy_calibration_current_target_excluded':True,'damage_averse_policy_survival':True,'shadow_baselines':['BALANCED','CURRENT_INCUMBENT_POLICY','RECENT_SURVIVING_POLICY'],'top5_damage_budget':True,'strength_tiers':['STRONG','MODERATE','WEAK','FAILING'],'minimum_tenure':'tier-dependent: 7/5/3/2',
    ...
    dump('primary_engine_integrity_report_v2_5.json',integrity)
    md=['# DHAPPA Dynamic Primary Engine v2.5 Report','',f"Dataset: **{rep['rows']} rows**, {rep['date_range'][0]} → {rep['date_range'][1]}",'',
        '## V2.5 election architecture','House-specific policy learning now uses Pareto survival against BALANCED, the current incumbent policy, and the recent surviving policy. A candidate policy is promoted only when it is non-inferior on protected metrics across all references and has strict prior-only gain; otherwise RETAIN_CURRENT_POLICY is emitted.','',
