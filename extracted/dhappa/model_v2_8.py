...
def run_walkforward(rows,min_train=45,max_combo=3,min_tenure=7,challenger_margin=0.010):
    ...
    meta_x=defaultdict(list); meta_y=defaultdict(list); discriminator_log=[]; shadow_events=defaultdict(list); boundary_log=[]; boundary_counts=Counter(); boundary_shadow=defaultdict(list)
    ...
    for i in range(min_train,len(rows)):
        ...
        for house in HOUSES:
            ...
            election_debug.append({'date':target['date'],'house':house,'context':ctx,'decision':decision_reason,'selected':primary_name,
                ...
                'qualification_boundary':{'qualified':sum(1 for r in routes if not r.get('boundary_rescued')),'near_tested':len(near_routes),'rescued':len(rescued),'rejected':rejected_count}})
    ...
    return evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug,calibration_events,policy_history,discriminator_log,shadow_events,boundary_log,boundary_counts,boundary_shadow
...
def report(rows,evals,combo_evals,timeline,switches,profiles,combo_profiles):
    ...
    rep['model']='DHAPPA Dynamic Primary Engine v2.8 — Qualification Boundary Rescue Lab'
...
def save_all(root,rows,evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug,calibration_events,policy_history,discriminator_log,shadow_events,boundary_log,boundary_counts,boundary_shadow):
    ...
    (out/'dynamic_primary_engine_backtest_v2_8.json').write_text(json.dumps(rep,indent=2),encoding='utf-8')
    (out/'dynamic_election_decisions_v2_8.json').write_text(json.dumps(election_debug,indent=2),encoding='utf-8')
    (out/'prospective_route_discriminator_v2_8.json').write_text(json.dumps(discriminator_log,indent=2),encoding='utf-8')
    (out/'prospective_route_discriminator_shadow_v2_8.json').write_text(json.dumps({h:v for h,v in shadow_events.items()},indent=2),encoding='utf-8')
    (out/'qualification_boundary_rescue_v2_8.json').write_text(json.dumps(boundary_log,indent=2),encoding='utf-8')
    (out/'qualification_boundary_summary_v2_8.json').write_text(json.dumps({h:{k:boundary_counts[(h,k)] for k in ('near_tested','rescued','near_rejected')} for h in HOUSES},indent=2),encoding='utf-8')
    (out/'qualification_boundary_shadow_v2_8.json').write_text(json.dumps({h:v for h,v in boundary_shadow.items()},indent=2),encoding='utf-8')
    modes={h:Counter(x['mode'] for x in discriminator_log if x['house']==h) for h in HOUSES}
    md=['# DHAPPA v2.8 — Qualification Boundary Rescue Lab','',
    ...
    (out/'DHAPPA_QUALIFICATION_BOUNDARY_RESCUE_V2_8_REPORT.md').write_text('\n'.join(md),encoding='utf-8')
    return rep
