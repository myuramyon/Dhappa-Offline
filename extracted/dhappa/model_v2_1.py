    ...
    for i in range(min_train,len(rows)):
        ...
        for house in HOUSES:
            ...
            if candidate is None:
                ...
                # incumbent lost qualification; challenger may replace without margin after minimum tenure.
                if tenure[house]>=min_tenure:
                    selected=candidate; decision_reason='INCUMBENT_FAILED_SURVIVAL'
                else:
                    selected=None; decision_reason='INCUMBENT_FAILED_BEFORE_MIN_TENURE_ABSTAIN'
                    no_primary[house]+=1
            elif candidate['name']==incumbent_name:
                selected=incumbent; decision_reason='CHAMPION_RETAINED'
            elif tenure[house]<min_tenure:
                selected=incumbent; decision_reason='HYSTERESIS_MIN_TENURE'
            elif candidate['score'] >= incumbent['score'] + challenger_margin and paired_proof and paired_proof['qualified']:
                selected=candidate; decision_reason='CHALLENGER_PAIRED_PROOF_PASSED'
            elif candidate['score'] >= incumbent['score'] + challenger_margin:
                selected=incumbent; decision_reason='CHALLENGER_PAIRED_PROOF_FAILED'
            else:
                selected=incumbent; decision_reason='CHALLENGER_MARGIN_FAILED'
            ...
            if selected is None:
                ...
                election_debug.append({'date':target['date'],'house':house,'context':ctx,'decision':decision_reason,'selected':primary_name,
                                       ...
                                       'routes_considered':len(routes),'paired_proof':paired_proof})
            else:
                ...
                election_debug.append({'date':target['date'],'house':house,'context':ctx,'decision':decision_reason,'selected':primary_name,
                                       ...
                                       'context_n':selected['context_n'],'context_mod':selected['context_mod'],'paired_proof':paired_proof})
...
def report(rows,evals,combo_evals,timeline,switches,profiles,combo_profiles):
    ...
    return {'model':'DHAPPA Dynamic Primary Engine v2.1','rows':len(rows),'date_range':[rows[0]['date'],rows[-1]['date']],
            ...
            'election_features':['champion_challenger_gating','15_30_60_expanding_agreement','minimum_tenure_hysteresis',
                                 'combination_survival_test','contextual_regime_modifier','NO_QUALIFIED_PRIMARY','paired_challenger_proof','champion_protection'],
...
def save_all(root,rows,evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug=None):
    ...
    dump('dynamic_primary_engine_backtest_v2_1.json',rep)
    dump('primary_engine_election_timeline_v2_1.json',timeline)
    dump('single_engine_performance_v2_1.json',{h:{e:window_metrics(v) for e,v in profiles[h].items()} for h in HOUSES})
    dump('engine_combination_performance_v2_1.json',{h:{e:window_metrics(v) for e,v in combo_profiles[h].items()} for h in HOUSES})
    dump('dynamic_election_decisions_v2_1.json',election_debug or [])
    failures=[]
    for x in timeline:
    ...
    dump('generation_vs_ranking_failure_v2_1.json',failures)
    integrity={'version':'v2.1','no_current_target_used_for_generation':True,'history_slice':'rows[:target_index]',
               'election_before_target_profile_update':True,'freeze_hash':'sha256(date,cutoff,house,engine,ranking)',
               'explicit_abstention_state':True,'paired_challenger_proof':True,'minimum_tenure':7,
    ...
    md += ['', '## Interpretation','V2.1 is allowed to abstain and blocks score-only challenger switches. Therefore conditional hit rates on elected targets must be read together with election coverage; lower coverage can inflate conditional metrics. The all-target Top-5 measure is retained in JSON to prevent selective-reporting bias. Random ranking references remain Top-5=5%, Top-10=10%, Top-21=21%, Top-36=36%.','',
           '## Integrity conclusion','V2.1 preserves the prior-only election ordering and adds paired challenger proof before incumbent replacement. Election and route qualification are now performed entirely from previously completed targets, followed by immutable freeze and only then outcome evaluation.']
    (out/'DHAPPA_DYNAMIC_PRIMARY_ENGINE_V2_1_REPORT.md').write_text('\n'.join(md),encoding='utf-8')
    return rep
