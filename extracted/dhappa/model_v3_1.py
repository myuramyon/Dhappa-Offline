...
def run_walkforward(rows,min_train=45,max_combo=3,min_tenure=7,challenger_margin=0.010):
    ...
    meta_x=defaultdict(list); meta_y=defaultdict(list); discriminator_log=[]; shadow_events=defaultdict(list); boundary_log=[]; boundary_counts=Counter(); boundary_shadow=defaultdict(list); tournament_log=[]; tournament_counts=Counter(); fusion_shadow=defaultdict(list); fusion_log=[]; candidate_rescue_shadow=defaultdict(list); candidate_rescue_log=[]
    ...
    for i in range(min_train,len(rows)):
        ...
        for house in HOUSES:
            ...
            if selected is None:
                primary_name='NO_QUALIFIED_PRIMARY'; primary_rank=None; canonical_primary_rank=None; primary_hash=None; secondary=None; rescue_proposal=None; candidate_attribution={}; rescue_status={'active':False,'stage':'NO_PRIMARY'}; rescue_active=False
                ...
                rescue_active=bool(rescue_proposal is not None and rescue_status.get('active'))
            ...
            if selected is not None and rescue_proposal is not None:
                ...
                candidate_rescue_log.append({'date':target['date'],'source_cutoff':cutoff,'house':house,'primary':primary_name,
                    'active':rescue_active,'status':rescue_status,'incumbent':rescue_proposal['incumbent'],
    ...
    return evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug,calibration_events,policy_history,discriminator_log,shadow_events,boundary_log,boundary_counts,boundary_shadow,tournament_log,tournament_counts,fusion_shadow,fusion_log,candidate_rescue_shadow,candidate_rescue_log
...
def report(rows,evals,combo_evals,timeline,switches,profiles,combo_profiles):
    ...
    rep['model']='DHAPPA v3.1 — Candidate Evidence Attribution & Rank-6–12 Rescue'
...
def save_all(root,rows,evals,combo_evals,timeline,switches,profiles,combo_profiles,election_debug,calibration_events,policy_history,discriminator_log,shadow_events,boundary_log,boundary_counts,boundary_shadow,tournament_log,tournament_counts,fusion_shadow,fusion_log,candidate_rescue_shadow,candidate_rescue_log):
    ...
    (out/'dynamic_primary_engine_backtest_v3_1.json').write_text(json.dumps(rep,indent=2),encoding='utf-8')
    ...
    (out/'candidate_rank6_12_rescue_v3_1.json').write_text(json.dumps(candidate_rescue_log,indent=2),encoding='utf-8')
    (out/'candidate_rank6_12_rescue_shadow_v3_1.json').write_text(json.dumps({h:v for h,v in candidate_rescue_shadow.items()},indent=2),encoding='utf-8')
    ...
    md[0]='# DHAPPA v3.1 — Candidate Evidence Attribution & Rank-6–12 Rescue'
    ...
    (out/'DHAPPA_CANDIDATE_RANK6_12_RESCUE_V3_1_REPORT.md').write_text('\n'.join(md),encoding='utf-8')
    return rep
