...
from dhappa.model_v2_4 import load_csv, run_walkforward
...
for p in policy_history:
    meta=p.get('meta',{})
    assert meta.get('reason') in {'WARMUP_DEFAULT','PRIOR_ONLY_DAMAGE_AVERSE_POLICY_SURVIVAL'}
    if meta.get('reason')=='PRIOR_ONLY_DAMAGE_AVERSE_POLICY_SURVIVAL':
        assert 'survival' in meta
print('v2.4 integrity PASS',len(timeline),'targets', {h:len(v) for h,v in calibration_events.items()})
